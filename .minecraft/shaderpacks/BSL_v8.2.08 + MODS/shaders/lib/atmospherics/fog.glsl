...
void NormalFog(inout vec3 color, vec3 viewPos) {
	...
	#ifdef OVERWORLD
	float fog = length(viewPos) * fogDensity / 1024.0;
	float clearDay = sunVisibility * (1.0 - rainStrength);
	fog *= mix(1.0, mix(1.0, FOG_DENSITY_WEATHER, rainStrength) / mix(1.0 / FOG_DENSITY_NIGHT, 1.0, clearDay) * eBS, eBS);
	...
	if(isEyeInWater == 0.0){
		...
		#endif
		float vanillaFog = 1.0 - (far - (fogFactor + fogOffset)) * 5.0 / (FOG_DENSITY_VANILLA * far);
	...
	#ifdef END
	float fog = length(viewPos) * fogDensity / 128.0;
	#if FAR_VANILLA_FOG == 2 || FAR_VANILLA_FOG == 3
